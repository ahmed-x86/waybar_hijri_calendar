...
def fetch_hijri_month(date_str):
    try:
        ...
        with urllib.request.urlopen(urllib.request.Request(url_today, headers={'User-Agent': 'Mozilla/5.0'}), timeout=5) as resp:
            data = json.loads(resp.read().decode('utf-8'))
            h_month, h_year = data['data']['hijri']['month']['number'], data['data']['hijri']['year']
        
        url_month = f"https://api.aladhan.com/v1/hToGCalendar/{h_month}/{h_year}"
        with urllib.request.urlopen(urllib.request.Request(url_month, headers={'User-Agent': 'Mozilla/5.0'}), timeout=5) as resp:
...
def main():
    ...
    if today_str not in cache:
        ...
        if new_data: cache.update(new_data); save_cache(cache)

    if today_str not in cache:
        print(json.dumps({"text": I18N[lang]["error"], "tooltip": I18N[lang]["no_connection"]}, ensure_ascii=False)); sys.exit(1)
